...
def search():
    ...
    for country in countries:
        ...
        if user == name.get_text(strip=True):
            capital_name = country.find('span', class_='country-capital').get_text(strip=True)
    ...
    try:
        ...
        if ratio < 0.5:
            ...
            return render_template('result.html', country=user, capital=capital_name, condition=condition, temperature=temperature, humidity=humidity, wind_speed=wind_speed)
